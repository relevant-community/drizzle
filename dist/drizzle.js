...
function createBlockPollChannel(_ref3) {
  ...
  return (0, _reduxSaga.eventChannel)(function (emit) {
    ...
    blockTracker.on('sync', function (_ref4) {
      var newBlock = _ref4.newBlock,
          oldBlock = _ref4.oldBlock;

      var newB = Number(newBlock);
      var oldB = Number(oldBlock);
      console.log('new block ', Number(newB));
      // console.log('old block ', Number(oldB))
      var skipped = oldB ? newB - oldB : 0;

      while (skipped > 1) {
        var _blockHeader = { number: oldB + 1 };
        emit({ type: 'BLOCK_RECEIVED', blockHeader: _blockHeader, drizzle: drizzle, web3: web3, syncAlways: syncAlways });
        skipped--;
        console.log('getting skipped blocks ', skipped, oldB + 1);
      }

      var blockHeader = { number: newB };
      emit({ type: 'BLOCK_RECEIVED', blockHeader: blockHeader, drizzle: drizzle, web3: web3, syncAlways: syncAlways });
    }).on('error', function (error) {
      emit({ type: 'BLOCKS_FAILED', error: error });
      emit(_reduxSaga.END);
...
function callCreateBlockPollChannel(_ref5) {
  var drizzle = _ref5.drizzle,
...
function processBlockHeader(_ref6) {
  var blockHeader = _ref6.blockHeader,
...
function processBlock(_ref7) {
  var block = _ref7.block,
      drizzle = _ref7.drizzle,
      web3 = _ref7.web3,
      syncAlways = _ref7.syncAlways;
